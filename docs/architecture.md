...
### Дедупликация: три выигрыша и один риск

Кластеризация входящих обращений по эмбеддингам даёт сразу три разных эффекта, которые полезно не смешивать: **стоимость** (одна генерация на кластер вместо 20k вызовов — главный рычаг экономии во всей системе), **обнаружение инцидента** (быстро растущий кластер сам по себе является детектом, часто более ранним, чем алерт на технической метрике) и **нагрузка на операторов** (инцидент-менеджер видит один кластер с суммаризацией, а не двадцать тысяч тикетов).

Ответ при инциденте принадлежит к отдельному типу: это не ответ из базы знаний, а статус — «проблема известна, работы идут, ориентировочное время». Это один из немногих случаев, где автоматический ответ одновременно безопасен и особенно ценен: подтверждение «мы в курсе» почти невозможно сформулировать неверно, а молчание при массовом сбое — худший из возможных сценариев для поддержки.

**Риск при этом принципиально иной, чем у обычного автоответа.** Дедупликация — операция с наибольшим радиусом поражения во всей системе: один неверно склеенный кластер означает один неверный ответ, отправленный тысячам людей одновременно. Поэтому участие человека здесь меняет форму: не «оператор проверяет каждый тикет», а **«оператор один раз утверждает канонический ответ, после чего он рассылается по кластеру»**. Порог склейки задаётся консервативно — разделить один инцидент на два кластера дёшево, склеить два разных в один дорого.
